  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "${aws_cloudwatch_log_group.app_servers.arn}",
        "${aws_cloudwatch_log_group.app_servers.arn}:log-stream:*"
      ]
    },
    {
      "Effect": "Allow",
...
resource "aws_iam_policy_attachment" "logging_app_access" {
  name = "logging_app_access_policy_attachment_${terraform.workspace}"
  roles = ["${aws_iam_role.s3_app_access_role.name}"]
